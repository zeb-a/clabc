...
const TornadoGameWrapper = ({ onBack, classes: externalClasses, isReplay: externalIsReplay }) => {
  ...
  // For liveworksheet and spelltheword, go directly to playing - they have their own built-in config
  const gameTypeFromStorage = localStorage.getItem('selected_game_type');
  const isLiveWorksheet = gameTypeFromStorage === 'liveworksheet';
  const isSpellTheWord = gameTypeFromStorage === 'spelltheword';
  const initialGameState = isLiveWorksheet || isSpellTheWord ? 'playing' : (isReplay ? 'select-class' : 'config');
  ...
  const [isDraggingSpellTheWord, setIsDraggingSpellTheWord] = useState(false);
            ...
            {classes.map(cls => (
              <div
                ...
                onClick={() => {
                  ...
                  // Skip mode selection for FaceOff, Memory Match, Quiz, MotoRace, HorseRace, Spell the Word — go directly to config
