...
import SpellTheWordGame from './SpellTheWordGame';
...
const TornadoGameWrapper = ({ onBack, classes: externalClasses, isReplay: externalIsReplay }) => {
  ...
  const handleSpellTheWordFileDrop = (files) => {
    const imageFiles = Array.from(files).filter(f => f.type.startsWith('image/'));
    const readers = imageFiles.map(file => {
      return new Promise(resolve => {
        const r = new FileReader();
  ...
  return (
    <div style={{
      ...
      {gameState === 'select-game' && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '20px', marginBottom: '20px', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.3s ease',
                background: gameType === 'spelltheword' ? 'linear-gradient(135deg, #EC4899, #8B5CF6)' : 'rgba(255,255,255,0.3)',
          ...
          <div style={{
            ...
            <div style={{ marginTop: '10px' }}><strong>🔤 Spell the Word:</strong> Spell words by clicking letters, with images and dual-player support!</div>
          </div>
        </div>
      )}
            ...
            {classes.map(cls => (
              <div
                ...
                onClick={() => {
                  ...
                  } else if (gameType === 'faceoff' || gameType === 'memorymatch' || gameType === 'quiz' || gameType === 'motorace' || gameType === 'horserace' || gameType === 'spelltheword') {
                    setGameState('config');
                  } else {
      ...
      {/* Game Configuration Screen - Spell the Word */}
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      ...
                      const words = text.split(/[,，\n]+/).map(w => w.trim()).filter(Boolean);
                      if (words.length) {
                        setSpellTheWordConfig(prev => ({ ...prev, words: [...prev.words, ...words.map(w => ({ id: Date.now() + Math.random(), word: w, image: null }))] }));
                        if (input) input.value = '';
                      }
                    }
                  }}
          ...
          {spellTheWordTab === 'images' && (
            <div
              ...
              onDrop={(e) => {
                ...
                const files = e.dataTransfer.files;
                if (files.length > 0) {
                  handleSpellTheWordFileDrop(files);
                }
              }}
              style={{ marginBottom: '18px', padding: '18px', background: isDraggingHorseRace ? '#FCE7F3' : '#f8fafc', borderRadius: '16px', border: isDraggingHorseRace ? '3px dashed #EC4899' : '2px solid #EC489940', transition: 'all 0.3s' }}
            >
              <label style={{ fontSize: '14px', fontWeight: '700', color: '#9F1239', display: 'block', marginBottom: '8px' }}>
                {t('games.upload_images_bulk')}
              </label>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: '16px',
                  fontWeight: '700',
                  border: isDraggingHorseRace ? '3px dashed #EC4899' : '2px dashed #EC4899',
                  borderRadius: '15px',
                  background: isDraggingHorseRace ? '#FCE7F3' : '#fff',
                  color: '#EC4899',
                  cursor: 'pointer',
                  marginBottom: '12px',
                  display: 'flex',
          ...
          <button
            ...
            onClick={() => {
              ...
              if (hasEnoughWords && hasEnoughPlayers) {
                setPlayers(selectedStudents.slice(0, playerCount));
                localStorage.setItem('players', JSON.stringify(selectedStudents.slice(0, playerCount)));
                localStorage.setItem('spelltheword_config', JSON.stringify({
