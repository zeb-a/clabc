...
export default function WeeklyTemplate({ data = {}, onChange }) {
  ...
  const updateRow = (index, field, value) => {
    const next = [...rows];
    if (!next[index]) next[index] = { day: WEEKLY_DAY_LABELS[index], focus: '', languageTarget: '', assessment: '' };
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#1e293b', color: '#fff', borderBottom: '2px solid #334155' }}>
              <th style={{ padding: '12px 14px', textAlign: 'left', fontWeight: 600 }}>Day</th>
          ...
          <tbody>
            {WEEKLY_DAY_LABELS.map((day, i) => (
              <tr key={day} style={{ borderBottom: '1px solid #E5E7EB', background: i % 2 === 0 ? '#fff' : '#f8fafc' }}>
                <td style={{ padding: '10px 14px', fontWeight: 600, color: '#475569', minWidth: 100 }}>{day}</td>
